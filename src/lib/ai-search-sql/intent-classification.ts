...
export const defaultInvoiceIntents: IntentSpec[] = [
  {
    ...
    examples: ["Show me all open invoices", "Give me the E.ON invoices", "Rechnungen von letztem Monat"],
  },
  {
    name: "total_amount",
  ...
  {
    name: "rank_breakdown",
    purpose: "User wants a ranking or per-group comparison across companies, suppliers, properties or categories.",
    ...
    properties: {
      ...
      entities: {
        ...
        properties: {
          ...
          documentType: { type: ["string", "null"], enum: ["invoice", "credit_note", "other", null] },
...
export function buildIntentClassificationInstructions(
  ...
  const propertyList =
    vocabulary.properties
      .map((property) => {
        const alias = property.aliases?.length ? `; also called: ${property.aliases.join(", ")}` : "";
  ...
  const categoryList =
    vocabulary.categories
      ...
      .join(", ") || "(none)";
  const supplierList =
    vocabulary.suppliers.slice(0, SUPPLIER_SAMPLE_LIMIT).join(", ") || "(none)";
