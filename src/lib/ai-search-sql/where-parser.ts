...
type TokenKind = "identifier" | "keyword" | "number" | "string" | "operator" | "lparen" | "rparen" | "comma";
  ...
  private literal(type: QueryColumnType): string {
    ...
    if (type === "number") {
      if (token.kind !== "number") {
        throw new WhereClauseError(`Expected a number for a numeric column, found "${token.value}".`);
      }
      return token.value;
    }
    if (type === "date") {
    ...
    if (type === "boolean") {
      if (token.kind !== "keyword" || (token.value !== "true" && token.value !== "false")) {
        throw new WhereClauseError(`Expected true or false for a boolean column, found "${token.value}".`);
      }
      return token.value;
    }
    if (token.kind !== "string") {
      throw new WhereClauseError(`Expected a quoted string for a text column, found "${token.value}".`);
    }
    return quoteString(token.value);
  }
  ...
  private parseComparison(): string {
    ...
    if (operatorToken.kind === "keyword" && operatorToken.value === "between") {
      ...
      if (spec.type !== "number" && spec.type !== "date") {
        throw new WhereClauseError(`BETWEEN is only allowed on number or date columns, not "${spec.name}".`);
      }
